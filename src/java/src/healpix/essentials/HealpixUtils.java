...
static public void tableprinter()
  {
...
String res=new String();
    for (int path=0; path<8; ++path)
      ...
      for (int pix=0; pix<16; ++pix)
        {
        int pix2=peano_subpix[dir][path][pix>>>2],
            path2=peano_subpath[dir][path][pix>>>2];
        int val=(pix2<<2) | peano_subpix[dir][path2][pix&0x3]
               +(peano_subpath[dir][path2][pix&0x3]<<4)
               +(dir<<7);
        res+=val+",";
        }
res+="\n";
  ...
  // classic version, two 3D lookup tables in the loop
  ...
  // fairly standard version, one 1D lookup table in the loop
  static private long nest_peano_helper2 (long pix, int order, int dir)
    {
    ...
    int face = (int)(pix>>>(2*order));
    long result = 0L;
    int state = (peano_face2path[dir][face]<<2)|(dir<<5);

    for (int shift=2*order-2; shift>=0; shift-=2)
      {
      state=arr[(state&0xFC) | (((int)(pix>>>shift))&0x3)];
      result = (result<<2) | (state&0x3);
      }

    return result + (((long)peano_face2face[dir][face])<<(2*order));
    }
  // 1D lookup table in the loop, do two steps at once as long as possible
  static private long nest_peano_helper3 (long pix, int order, int dir)
    {
    final short arr2[] = {
  ...
  // no lookup tables in the loop, use long integers instead.
  // This version could be vectorized, otherwise it's probably worthless
  static private long nest_peano_helper4 (long pix, int order, int dir)
    {
    final byte peano_face2path[][] =
      { { 2,5,2,5,3,6,3,6,2,3,2,3 }, { 2,6,2,3,3,5,2,6,2,3,3,5 } };
    final byte peano_face2face[][] =
    ...
    int face = (int)(pix>>>(2*order));
    long result = 0L;
    int state = (peano_face2path[dir][face]<<2);
    if (dir==1)
      {
      final long a=2199974852034244020L, b=9163371139704739200L,
                 c=1446803457519468865L;
      for (int shift=2*order-2; shift>=0; shift-=2)
        {
        state=(state&0xFC)|((int)(pix>>>shift) & 0x3);
        state=(state&0x3)|((int)((b>>>(2*state))&0x3)<<2)
                         |((int)((c>>>(2*state))&0x3)<<4);
        result = (result<<2) | (a>>>(2*state))&0x3;
        }
      }
    else
      {
      final long a=5419767265092330420L, b=-177095002849913056L,
                 c=4901342806104343825L;
      for (int shift=2*order-2; shift>=0; shift-=2)
        {
        state=(state&0xFC)|((int)(pix>>>shift) & 0x3);
        state=(state&0x3)|((int)((b>>>(2*state))&0x3)<<2)
  ...
  static public long nest2peano(long pix, int order)
    { return nest_peano_helper3(pix,order,0); }
  static public long peano2nest(long pix, int order)
    { return nest_peano_helper3(pix,order,1); }
