...
public class HealpixIndexNew implements Serializable {
...
static private final short utab[]={
  ...
  21824,21825,21828,21829,21840,21841,21844,21845 };
	 ...
	 * @throws Exception
	 */
	public HealpixIndexNew(int nSIDE2) throws Exception {
	...
	public HealpixIndexNew(int nside2, Scheme sch) throws Exception{
		if ( nside2 > ns_max || nside2 < 1 ) {
			throw new Exception("nsides must be between 1 and " + ns_max);
		}
		this.nside = nside2;
	...
	public long ang2pix_nest(double theta, double phi) throws Exception {
		...
		long pix = zphi2pix(Math.cos(theta),phi);
		scheme=sbak;
		return pix;
	}
	...
	public long vec2pix_nest(SpatialVector vec) throws Exception {
		Scheme sbak=scheme;
		scheme=Scheme.NESTED;
		long pix = zphi2pix(vec.z()/vec.length(),Math.atan2(vec.y(),vec.x()));
		scheme=sbak;
		return pix;
	}
	/**
	...
	public void queryDisc(AngularPosition ptg,
		  ...
		  if (scheme==Scheme.RING)
		    ...
		    HealpixIndexNew[] base = new HealpixIndexNew[omax+1];
...
public long zphi2pix (double z, double phi)
  {
final double inv_halfpi=0.6366197723675813430755350534900574;

  double za = Math.abs(z);
  double tt = Math.IEEEremainder((phi*inv_halfpi),4.0); // in [0,4)
  if (tt<0) tt+=4.0;
  ...
  if (scheme==Scheme.RING)
    ...
    if (za<=z0) // Equatorial region
      ...
      long ifm = jm >> order;
      long face_num = (ifp==ifm) ? ((ifp&3)+4) : ((ifp<ifm) ? ifp : (ifm+8));
      ...
      return (z>=0) ?
        xyf2nest((int)(nside-jm -1),(int)(nside-jp-1),ntt) : xyf2nest((int)jp,(int)jm,ntt+8);
      }
    }
  }
