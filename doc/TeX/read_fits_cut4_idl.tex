...
, \mylink{idl:read_fits_cut4:HELP}{HELP=}%
]
}
\end{IDLformat}
...
\begin{qualifiers}
  \begin{qulist}{} %%%% NOTE the ``extra'' brace here %%%%
 	\item[{File}]  \mytarget{idl:read_fits_cut4:File}
          name of a FITS file in which the map is to be written

 	\item[{Pixel}]  \mytarget{idl:read_fits_cut4:Pixel}
	 (OUT, LONG vector), \\ index of observed (or valid) pixels

 	\item[{Signal}]  \mytarget{idl:read_fits_cut4:Signal}
	 (OUT, FLOAT vector), \\ value of signal in each observed pixel

 	\item[{N\_Obs}]  \mytarget{idl:read_fits_cut4:N_Obs}
	 (OUT, LONG or INT vector, Optional), \\ number of observation per pixel

 	\item[{Serror}]  \mytarget{idl:read_fits_cut4:Serror}
...
and U Stokes parameters related information. \default{0} 
	 ...
	 \item[{HELP=}] \mytarget{idl:read_fits_cut4:HELP}
