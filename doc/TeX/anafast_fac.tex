...
${\cal {O}}(N_{\textrm{pix}}^{3/2} )$ with a prefactor
depending on $\lmax$. %
\\ %
...
$\lmax \approx 2\cdot nsmax$, with a very good $C_\ell$ 
error performance 
...
system up to $\lmax = 3 \cdot nsmax -1$. Hence, the functions which are
strictly band-width limited to $\lmax = 3 \cdot nsmax - 1 $ 
can be fully
...
--- $2 \cdot nsmax < \lmax < 3 \cdot nsmax - 1$ --- is easily
