...
{\tt CBAR.BOX}: thickness of the black box drawn around the color bar \default{0: no box}; 
...
{\tt VSCALE.TY}: vertical offset of the text next to the calibrating rod.\\
...
\htmladdnormallink{ImageMagick {\tt convert}}{https://www.imagemagick.org/script/index.php}
...
one meta-structure ({\tt outline=\{s1:circle,s2:triangle\}}) in all cases.
\end{minipage}}}
\\
...
or {\tt orthographic}, \\
	      if set to a file name  : outputs the plot in that file \\
		\default 0 \\
The PDF file is produced from a PostScript file using the script \htmladdnormallink{{\tt epstopdf}}{https://tug.org/epstopdf/} now shipped with \healpixns. Note that {\tt epstopdf} usually requires a fully functional implementation of the fairly widespread {\tt gs}, aka
...
\htmladdnormallink{{\tt pdfjam}}{https://www2.warwick.ac.uk/fac/sci/statistics/staff/academic-research/firth/software/pdfjam/}, and/or 
