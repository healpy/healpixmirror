...
the final thickness is \texttt{2* cbar.box*!p.thick} in 
\mylink{idl:mollview:pdf}{PDF} and 
\mylink{idl:mollview:ps}{PS}, 
and \texttt{cbar.box*!p.thick} otherwise, 
where \texttt{!p.thick} is assumed to have a value of 1.0 unless specified otherwise.\\
...
{SILHOUETTE=}\mytarget{idl:mollview:silhouette} & \mylink{idl:mollview:routines}{---MO}  & \parbox[t]{\hsize}{
  if set to a scalar or 2-element vector with silhouette[0] $\ne 0$,
          ...
          Its thickness is proportional to abs(silhouette[0])
           (and also depends in a device dependent manner on \texttt{!P.THICK})\\
