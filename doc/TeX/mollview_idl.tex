...
structure) containing the (RING or \mylink{idl:mollview:nested}{NESTED} ordered) \healpix\ map (See note below);\\
...
format, and the \mylink{idl:mollview:polarization}{\texttt{POLARIZATION}} IDL keyword
...
\seealso \mylink{idl:mollview:truecolors}{\texttt{TrueColors}}.}\\
...
Select \mytarget{idl:mollview:select}  & \mylink{idl:mollview:routines}{all}  & \parbox[t]{0.95\hsize}{Optional\\
		  ...
		  \mylink{idl:mollview:file}{\texttt{File}} is an online array) \\
...
$y \approx 0.21 x$ when $x\ll 1$ and $y\approx \log(x)$ when $x\gg 1$. \\
	...
	\mylink{idl:mollview:factor}{\texttt{Factor}} and \mylink{idl:mollview:offset}{\texttt{Offset}}. \\
            This option can {\em not} be used in conjonction with
\mylink{idl:mollview:log}{\texttt{/LOG}} nor \mylink{idl:mollview:hist_equal}{\texttt{/HIST\_EQUAL}}.}\\
...
\htmlref{\texttt{!healpix.bad\_value}}{idl:init_healpix} ($=-1.6375\,10^{30}$) 
...
\mylink{idl:mollview:glsize}{graticule labels}), can be controlled with \texttt{!P.CHARTHICK}.
                \default {1} \\
...
reserved for Planck color tables 1 and 2 generated by \htmlref{\texttt{planck\_colors}}{idl:planck_colors}. 
...
\texttt{ASPOS.X, ASPOS.Y}: X,Y location of astronomical coordinates label  (which can be removed altogether with \mylink{idl:mollview:noposition}{/NOPOSITION}, only applicable to \htmlref{gnomview}{idl:gnomview}),\\
\texttt{CBAR.DX, CBAR.DY}: length and width of color bar (which can be removed with \mylink{idl:mollview:nobar}{/NOBAR}), the bar is centered, at an automatically determined height\\
\texttt{CBAR.SPACES}: 3-element vector listing the strings to be inserted between the map minimum value label, the color bar, the map maximum value, and the map units (read from the FITS file or provided by \mylink{idl:mollview:units}{UNITS}) \default{3 single spaces}, \\
\texttt{CBAR.TY}: vertical offset of the text (min, max and units labels) with respect to the color bar \default{0};\\
...
\texttt{CBAR.BOX}: thickness of the black box drawn around the color bar \default{0: no box}; 
the final thickness is \texttt{F*cbar.box*!p.thick} with \texttt{F}=2 in
\mylink{idl:mollview:pdf}{PDF} and 
\mylink{idl:mollview:ps}{PS}, or \texttt{F}=1 otherwise, 
while \texttt{!p.thick} is assumed to have a value of 1.0 unless specified otherwise.\\
\texttt{CRING.DX, CRING.XLL, CRING.YLL}: radius and X,Y location of lower left corner of the color disc showing the polarization direction when \mylink{idl:mollview:polarization}{POLARIZATION=3}\\
\texttt{PDF.DEBUG}: if set to 1, and \mylink{idl:mollview:silent}{SILENT} is not set, then debugging information on the \mylink{idl:mollview:pdf}{PDF} generation will be printed (when applicable), 
...
As shown in \mylink{idl:mollview:example7}{Example \#7} \latexhtml{on page~\pageref{page:moll_customize}}{below}, the returned structure can be inspected with the routine \htmlref{\texttt{help\_st}}{idl:help_st}.}\\
...
\htmladdnormallink{STIFF}{https://www.astromatic.net/category/softwareposts/stiff})%
...
{IGLSIZE=}\mytarget{idl:mollview:iglsize} & \mylink{idl:mollview:routines}{CGMO}& \parbox[t]{\hsize}{
	character size of the input coordinates graticule labels in units of \texttt{Charsize}. \\
...
{IGRATICULE=}\mytarget{idl:mollview:igraticule}  & \mylink{idl:mollview:routines}{CGMO}  & \parbox[t]{\hsize}{
	...
	The automatic labeling of the graticule is controlled by \texttt{Iglsize}\\
	\default {0 [no graticule]} \\
...
\htmladdnormallink{ImageMagick \texttt{convert}}{https://www.imagemagick.org/script/index.php}
and/or \htmladdnormallink{GraphicsMagick \texttt{gm convert}}{http://www.graphicsmagick.org/convert.html}
...
\mylink{idl:mollview:coord}{COORD} respectively, ... \\
%
Unobserved pixels, and pixels outside the sphere, take value \htmlref{\texttt{!healpix.bad\_value}}{idl:init_healpix} ($=-1.6375\,10^{30}$).\\
...
scalar)\\
	and can optionally contain the fields: \\
            -- 'COL[OR]': (optional, scalar or 2-element vector) index in [0,255] of the colors used to draw the lines and symbols. \default{[!p.color, !p.background], ie black and white}\\
...
or \texttt{orthographic}, \\
	      if set to a file name  : outputs the plot in that file \\
		\default 0 \\
The PDF file is produced from a PostScript file using the script \htmladdnormallink{\texttt{epstopdf}}{https://tug.org/epstopdf/} now shipped with \healpixns. Note that \texttt{epstopdf} usually requires a fully functional implementation of the fairly widespread \texttt{gs}, aka
\htmladdnormallink{\texttt{Ghostscript}}{https://en.wikipedia.org/wiki/Ghostscript}, which may however not be available on the computation dedicated nodes of some computer clusters.\\
...
\htmladdnormallink{\texttt{pdf90}}{https://linux.die.net/man/1/pdf90}, part of the package 
\htmladdnormallink{\texttt{pdfjam}}{https://www2.warwick.ac.uk/fac/sci/statistics/staff/academic-research/firth/software/pdfjam/}, and/or 
\htmladdnormallink{\texttt{pdfcrop}}{http://pdfcrop.sourceforge.net}, 
...
\texttt{!p.font=-1} and prepending the
\mylink{idl:mollview:subtitle}{Subtitle},
\mylink{idl:mollview:titleplot}{Titleplot} and
\mylink{idl:mollview:units}{Units} 
strings with \texttt{'!6'}}.\\
...
{PNG=}\mytarget{idl:mollview:png}  & \mylink{idl:mollview:routines}{all}  & \parbox[t]{\hsize}{
		string containing the name of a .PNG output \\
	      if set to 1            : outputs the plot in 
\texttt{plot\_}{\em projection}\texttt{.png}, where {\em projection} is either
\texttt{azimequid}, \texttt{cartesian}, \texttt{gnomic}, \texttt{mollweide} 
or \texttt{orthographic}, \\
	      if set to a file name  : outputs the plot in that file \\
...
{POLARIZATION=}\mytarget{idl:mollview:polarization} & \mylink{idl:mollview:routines}{all}& \parbox[t]{\hsize}{
	if set to
% 	\begin{itemize}
% 	 \latexhtml{\setlength{\itemsep}{0pt}\setlength{\leftmargin}{.1cm}\setlength{\labelwidth}{.1cm}}{}
%         \item[0:] 
...
%         \item[3:]
	\\3:
             ...
             (which also depends in a device dependent manner on \texttt{!P.THICK}) \default{1}. 
...
{SILHOUETTE=}\mytarget{idl:mollview:silhouette} & \mylink{idl:mollview:routines}{---MO}  & \parbox[t]{\hsize}{
  if set to a scalar or 2-element vector with silhouette[0] $\ne 0$,
          a silhouette is drawn around the map.\\
          Its thickness is \texttt{F*abs(silhouette[0])*!P.THICK} with \texttt{F}=2 in
\mylink{idl:mollview:pdf}{PDF} and 
\mylink{idl:mollview:ps}{PS}, or \texttt{F}=1 otherwise.\\
          Its color is determined by abs(silhouette[1]) in [0,255] \default{0:\mylink{idl:mollview:fg_color}{FG\_COLOR}}.\\
See \mylink{idl:mollview:example7}{Example \#7} \latexhtml{on page~\pageref{page:moll_customize}}{below}.}\\
...
{TRANSPARENT=}\mytarget{idl:mollview:transparent}  & \mylink{idl:mollview:routines}{all}  & \parbox[t]{\hsize}{
		If set to 1, the input data pixels with value \htmlref{\texttt{!healpix.bad\_value}}{idl:init_healpix} ($=-1.6375\,10^{30}$) 
...
%,png='~/healpix_svn/doc/TeX/fig/moll_customize2.png',/preview
...
and modified location of the title (see Fig.~\ref{fig:moll_customize}\latexhtml{ on page~\pageref{page:moll_customize}}{}). 
The default value (in the Mollweide projection) 
 of the available customization parameters is also listed as \\
%{\scriptsize{
\begin{scriptsize}
\texttt{ % put font size before font type to get line spacing right
   ** Structure <.....>, 7 tags, length=128, data length=122, refs=1:
\begin{tabular}{lll}
...
\latexhtml{%for latex
\vskip -1cm
\centerline{\includegraphics[width=0.49\textwidth]{fig/moll_customize2}}
}{%for html
\centerline{\includegraphics[width=520pt]{fig/moll_customize2}{}}
}
\caption{%
