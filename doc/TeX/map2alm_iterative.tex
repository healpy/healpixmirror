...
and the current residual map $\left(\bw.\bm - \bw . \bS .\ba^{(k)}\right)$ is printed out, with these numbers expected
...
zbounds\mytarget{sub:map2alm_iterative:zbounds}(1:2), \hskip 6cm OPTIONAL  & DP & IN & section of the map on which to perform the $a_{\ell m}$
                   ...
                   \cos(\theta).$ \input{zbounds_sub}. If absent, the whole map is
analyzed\\
%
