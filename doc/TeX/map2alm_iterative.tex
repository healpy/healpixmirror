...
\textbf{name~\&~dimensionality} & \textbf{kind} & \textbf{in/out} & \textbf{description} \\ \hline
                   &   &   &                           \\ %%% for presentation
nsmax\mytarget{sub:map2alm_iterative:nsmax} & I4B & IN & the $N_{side}$ value of the map to analyse. \\
...
iter\_order\mytarget{sub:map2alm_iterative:iter_order} $=0$ is a straight analysis, while iter\_order $=3$ is usually a
good compromise. \\
%
...
%
alm\_TGC\mytarget{sub:map2alm_iterative:alm_TGC}(1:p, 0:nlmax, 0:nmmax) & SPC/ DPC & OUT & The $a_{lm}$ values output
from the analysis. 
...
%
zbounds\mytarget{sub:map2alm_iterative:zbounds}(1:2), \hskip 6cm OPTIONAL  & DP & IN & section of the map on which to perform the $a_{lm}$
