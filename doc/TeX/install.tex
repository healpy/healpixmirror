...
\centerline{\includegraphics[width=0.5\textwidth]{fig/new_dir_tree}}
...
The \texttt{libsharp} C library for optimized Spherical Harmonics operations, 
is used by the C++, Fortran and healpy packages. 
Starting with \mylink{install:v360}{version 3.60}, a new, faster, implementation is used, 
and will be configured (only once) at the same time as any of these packages,
...
\subsubsection[General]{General}
\begin{itemize}
\item Implementation of a faster \mylink{install:libsharp:config}{\texttt{libsharp}}
C-written library for the Spherical Harmonics computations in C++ and F90, 
reducing for instance the computation time of a map synthesis or analysis 
...
\subsubsection[C++]{\htmladdnormallink{C++}{\healpixwebpage/html/index_cxx.htm}}
\begin{itemize}
  \item Link to the new \mylink{install:libsharp:config}{\texttt{libsharp}} library
  \item Simpler configuration \mylink{install:cpp_config}{with the systematic use of \texttt{autoconf} tools}
