...
provided the script (possibly customized with environment variables described below)
where {\it <list>} is a comma separated list of items to be configured, to be chosen (and combined)
among
\begin{itemize}
\item    \texttt{all}:     configures everything, \\
...
\item    \texttt{cxx}:     configures C++ and the required items, same as \texttt{profile,sharp,cxx}
\item    \texttt{f90}:     configures F90 and the required items, same as \texttt{profile,sharp,f90}
\item    \texttt{idl}:     configures IDL and the required items, same as \texttt{profile,idl}
\item    \texttt{sharp}:   configures libsharp (required by F90 and C++)
\item    \texttt{profile}: configures user or system configuration files (required C,C++,F90,IDL)
...
The online help \texttt{./configure~-h} can be used to know the current value of these variables.
\end{itemize}
