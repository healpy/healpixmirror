...
sourced as described in \S~\ref{subsub:conf}, the aliases {\tt hidl}, {\tt hidlde} (and {\tt hgdl})
...
IDL (and/or GDL) can be tested by invoking {\tt hidl} or {\tt hidlde}  (resp. {\tt hgdl}).
...
while
{\tt make tidy} should be used between successive (failed) attempts with different compilers, compiler versions or compiler options, to avoid any conflict between new and pre-existing files.
...
11.* to 15.*\footnote{problems have been reported with one of the code ({\tt sky\_ng\_sim}) when compiled with
...
See the \htmlref{eponymous section}{idl:other_idl_libs} in the \linklatexhtml{``IDL Facilities Overview''}{idl.pdf}{idl.html}
...
satisfactory results\footnote{All the caveats listed below have been noticed in
GDL v0.9.3
...
\htmladdnormallink{\tt ImageMagick}{http://www.imagemagick.org}
...
\subsubsection*{\healpix with GDL status update}
...
However, they are clearly identified and actively looked at by the GDL 
development team and most of them
should be fixed in the forthcoming GDL 0.9.6 (expected late 2015).
...
\subsection{Bug corrections and Improvements in Version \hpxversion}%(2015-??)
...
\subsubsection[Fortran]{%
	...
	\linklatexhtml{subroutines}{subroutines.pdf}{subroutines.htm}}
	\begin{itemize}
	\item \htmlref{{\tt anafast}}{fac:anafast} routine now produces nine spectra 
	(TT, EE, BB, TE, TB, EB, ET, BT and BE, instead of six previously)
          when analyzing two polarized maps;
	\item \htmlref{{\tt alm2cl}}{sub:alm2cl} facility can now produces nine spectra 
...
%
\item a \healpix enabled GDL command ({\tt hgdl}) is defined during the 
...
\mylink{idl:bin_llcl}{{\tt bin\_llcl}},
\mylink{idl:query_disc}{{\tt query\_disc}}.
%
  \item update of the required
    \htmladdnormallink{{\tt IDL-astron} library}{http://idlastro.gsfc.nasa.gov/homepage.html}
    routines, and their supporting \htmladdnormallink{\tt Coyote}{http://www.idlcoyote.com}
