...
\subsection{Bug corrections and Improvements in Version \hpxversion}%2021-04??
...
\begin{itemize}
\item Drop support for Python 2.7-3.5
\item Improvements of the build system
\item Automatically build wheels for Linux/MacOS on Github actions
\item `write\_map` keeps dtype of input map array instead of float32 ; `read\_map` keeps dtype of FITS file instead of upcasting to float64 ; `write\_cl` uses dtype of input cl instead of float64
\item Flip sign for spin-0 `alm2map\_spin` and `map2alm\_spin` ; fixed `map2alm\_spin` bug for masked input
\item Support transparency in plotting with the `alpha` parameter
\item Experimental `projview` function to plot maps using projections from `matplotlib`
\item Removed the note that we will change order of cl in `synfast` and `synalm`, we will leave `new=False` default
\item Support nested maps `hp.smoothing`; fixed indexing issue in `bl2beam`
\item Changed all warnings to using the `logging` module, deprecated all `verbose` keywords
