...
GIF, JPEG, PDF, Postscript and PNG files \mylinkext{idl:mollview:preview}{created by IDL (or GDL)}. \\
If the \healpix configuration file is
sourced as described in \S\ref{subsub:conf}, the aliases \texttt{hidl}, \texttt{hidlde}, \texttt{hgdl} and \texttt{hgdlde}
...
information on using \healpix IDL/GDL together with other IDL libraries, and \S\ref{install:using_gdl} on using GDL instead of IDL.
...
IDL (and/or GDL) can be tested by invoking \texttt{hidl} or \texttt{hidlde}  (resp. \texttt{hgdl} or \texttt{hgdlde}).
...
{\em on} GDL directly to GDL developpers at 
...
\item the \mylinkext{idl:mollview:transparent}\texttt{transparent} keyword will be ignored in the
production of \mylinkext{idl:mollview:png}{PNG} files under GDL (except when \mylinkext{idl:mollview:truecolors}\texttt{TrueColors} is set and $N$x3 map is provided), 
and for the same reason, the \htmlref{\texttt{hpx2gs}}{idl:hpx2gs} routine won't mark missing pixels as
transparent in the output PNG file;
\item  and finally, the \mylinkext{idl:mollview:pfonts}{\texttt{pfonts}} keyword will not allow the selection of other fonts than Hershey vectorial fonts (pfonts[0]=-1).
...
\subsection{Bug corrections and Improvements in Version \hpxversion}%(2017-?)
...
%
\item \htmlref{\texttt{median\_filter}}{fac:median_filter} facility: faster by moving an internal array from heap to stack;
...
\subsection{Bug corrections and Improvements in Version 3.31}%(2016-08-26)
