...
\subsection{./configure [-L] [\doubledash auto=<list>]}
\label{sub:configure}
...
\item If the environment variables 
      \texttt{CC}                 (relevant for C/C++/F90/python/sharp),
...
as default values in the (interactive or automated) configuration process.\\
As \mytarget{install:configure:boolean} detailed futher down, several of these variables are boolean in nature, with the values
\texttt{1}, \texttt{y}, \texttt{Y}, \texttt{t} or \texttt{T} meaning 'yes',
and 
\texttt{0}, \texttt{n}, \texttt{N}, \texttt{f} or \texttt{F} meaning 'no'.\\
The online help \texttt{./configure~-h} can be used to show the current value of these variables,
which are inter-dependent and depend on the operating system.
\end{itemize}
...
(or set the \mylink{install:configure:boolean}{boolean} environment variable \texttt{PROFILE\_EDIT} to a negative value),
...
can be used to customize the whole process.\\
...
The environment variables \texttt{CC} and \texttt{SHARP\_COPT} can be used to pick the C compiler
and its options proposed during the interactive or automated configuration process.\\
...
\subsubsection[General]{General}
\begin{itemize}
\item The computation time of a map synthesis or analysis is reduced
