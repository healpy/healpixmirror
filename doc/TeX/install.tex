...
\item Other input fiducial $C(\ell)$, all in $(\mu$K$)^2$ and defined on the multipole range $[0, \ell_\textrm{max}]$, have been included for convenience in 
{\tt Healpix\_\hpxversion/test/} in a \healpix compatible format.
\\
\begin{tabular}{l l l l} 
\hline 
  \textbf{File name} & \textbf{alias} & \textbf{Origin} & $\ell_\textrm{max}$\\ \hline
...
\subsubsection[Fortran]{%
	...
	\linklatexhtml{subroutines}{subroutines.pdf}{subroutines.htm}}
\begin{itemize}
\item {\tt median\_filter} facility: faster by moving an internal array from heap to stack;
