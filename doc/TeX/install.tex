...
The \mylink{install:configure:boolean}{boolean} variable \texttt{CXX\_PARAL}, introduced in version 3.71 and defaulting to 1 (=true), controls whether the code will be parallelized (with OpenMP) or not. To obtain a serial implementation of the code, set \texttt{CXX\_PARAL=0} and make sure no OpenMP related flags appear in \texttt{CXXFLAGS}.\\
...
The \mylink{install:configure:boolean}{boolean} variable \texttt{SHARP\_PARAL}, introduced in version 3.71 and defaulting to 1 (=true), controls whether the library will be parallelized (with OpenMP) or not. To obtain a serial implementation of \texttt{libsharp}, set \texttt{SHARP\_PARAL=0} and make sure no OpenMP related flags appear in \texttt{SHARP\_COPT}.\\
...
\subsection{Bug corrections and Improvements in Version \hpxversion}%2020-09??
...
\subsubsection[General]{General}
\begin{itemize}
\item addition of \mylink{install:libsharp:config}{\texttt{SHARP\_PARAL}} and 
\mylink{install:cpp_config}{\texttt{CXX\_PARAL}} to control the parallel implementation of the \texttt{libsharp} library and C++ library and codes.
\end{itemize}
\subsubsection[Fortran 90]{Fortran 90 facilities and subroutines}
\begin{itemize}
\item Improvement of \htmlref{{\tt query\_disc}}{sub:query_disc} routine in \mylinkext{sub:query_disc:inclusive}{\tt inclusive} mode,
\item correction of bugs preventing the compilation with versions 10.* of {\tt gfortran},
...
\begin{itemize}
\item Improvement of \htmlref{{\tt query\_disc}}{idl:query_disc} routine in \mylinkext{idl:query_disc:inclusive}{\tt inclusive} mode
\end{itemize}
