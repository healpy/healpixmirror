...
of {\tt python} and few supporting Python libraries) but it will perform
...
\textbf{Java} and \textbf{Python} routines see table~\ref{tab:allpackages}}
...
The \healpix \textbf{Fortran 90}, \textbf{C++} and \textbf{C} distributions also require the publicly available CFITSIO
library:

\begin{tabular}{p{0.3\hsize} p{0.6\hsize}} \hline  
  \textbf{Software Package} & \textbf{Source} \\ \hline
                            &                          \\ %%% for presentation
  CFITSIO V 3.xx library            & \htmladdnormallink{{\tt
...
the {\tt
include/fitsio.h} include file (therefore {\tt /usr/local} often is the right
choice) and
...
\subsection{Bug corrections and Improvements in Version 3.0} % 2012-??
...
"healpix.newcore" package. It is strongly recommended to use this package
directly in future projects making use of Java HEALPix.
"healpix.newcore" is a port of the Healpix C++ library and presents a very
similar interface.

The "healpix.core" package is still provided. It uses "healpix.newcore"
...
than calling "healpix.newcore" methods directly, because of the necessary
data conversion.
