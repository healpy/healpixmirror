...
\subsubsection*{CFITSIO, Fortran codes and Apple's ARM chips}
If \healpixns-F90  codes (or any other Fortran code) are linked with cfitsio on Apple's
ARM chips (M1 and M2), then version 4.1.0 (March 2022) or above of cfitsio must
be used, see Section~\ref{sec:requirements}.
...
(like \texttt{xloadct}%, or \texttt{xyouts} with TrueType fonts
) and the restrictions listed below.
...
\subsubsection[C]{\linklatexhtml{C}{csub.pdf}{csub.htm}}
\begin{itemize}
\item Corrected documentation of \htmlref{\texttt{pix2vec}}{csub:pix_tools} routines
\end{itemize}
\subsubsection[C++]{\htmladdnormallink{C++}{\healpixwebpage/html/index_cxx.htm}}
\begin{itemize}
\item Cfitsio 4.1.0 (March 2022) and higher now properly supported
\end{itemize}
...
\item Bug corrections in
 ...
 \item added a patch for a bug detected in Apple-ARM-chips implementation of gfortran 11.
 \item Note that cfitsio 4.1.0 or higher is required for Fortran codes running on Apple's ARM chips
\end{itemize}
\subsubsection[IDL]{\linklatexhtml{IDL}{idl.pdf}{idl.htm}}
\begin{itemize}
\item bug corrections in
  \htmlref{\texttt{read\_fits\_map}}{idl:read_fits_map} and
...
\begin{itemize}
\item Switch to 
  \htmladdnormallink{\texttt{healpy 1.16.1}}{\hpysite/releases} 
  (\htmladdnormallink{CHANGELOG}{\hpysite/blob/master/CHANGELOG.rst})
  \begin{itemize}
  \item Updated CFITSIO included in healpy to 4.1.0, necessary for compatibility with Apple ARM chips
  \item Update HEALPix C++ sources to revision 1206 (just maintenance commits)
  \item Do not normalize binary arrays
  \item Fix unncessary log warning message in plots
  \item Fixed double application of margins in visualization functions when using subplot syntax and implemented margins parameter for mollview, orthview, and azeqview when subplot syntax is not used
  \item Fixed reuse\_axes=True for cartview and gnomview
  \item New features in projview: subplots, remove monopole-dipole, labels, tickmarks, graticule, Planck and WMAP colormaps
  \item Fixed the CFITSIO version mismatch warning
...
\subsubsection[C++]{C++}
\begin{itemize}
 \item Cfitsio 4.0.0 (Jul 2021) and higher now properly supported
\end{itemize}

\subsubsection[IDL]{IDL}
\begin{itemize}
 \item Introduced workarounds for some minor GDL and FL limitations.
\end{itemize}

\subsubsection[Python]{Python}
\begin{itemize}
\item Switch to 
	\htmladdnormallink{\texttt{healpy 1.15.2}}{\hpysite/releases} 
	(\htmladdnormallink{CHANGELOG}{\hpysite/blob/master/CHANGELOG.rst})
\begin{itemize}
    \item new function {\texttt{blm\_gauss}} to generate alm of a gaussian beam,
    \item implement rotation in the graticule of {\texttt{projview}},
    \item explain how to create a local datapath for pixel weights,
    \item improvement on is\_seq to avoid {\texttt{synalm}} breaking on JAX input arrays, added unit tests,
    \item upgraded HEALPix C++ sources to HEALPix 3.81, fixing incompatibility with CFITSIO 4
\end{itemize}
\end{itemize}

\subsection{Bug corrections and Improvements in Version 3.80}%2021-06-22
\label{sec:v380}\mytarget{install:v380}
\label{sec:v371}\mytarget{install:v371}
\subsubsection[General]{General}
\begin{itemize}
\item addition of 
\mylink{install:libsharp:config}{\texttt{SHARP\_PARAL}} and 
\mylink{install:cpp_config}{\texttt{CXX\_PARAL}} 
to control the parallel implementation of the \texttt{libsharp} library and C++ library and codes;
\item \mylink{install:healpy:config}{\texttt{PYTHON}} now defaults to \texttt{python3}
\end{itemize}
%
% % \subsubsection[C++]{\htmladdnormallink{C++}{\healpixwebpage/html/index_cxx.htm}}
\subsubsection[C++]{C++}
\begin{itemize}
\item the line-integral convolution interface is now accessible not only from the command line, but also via C++ calls, to allow calling from \texttt{healpy};
\item some internals were restructured to allow easier integration with \htmladdnormallink{\texttt{SWIG}}{http://www.swig.org}
\end{itemize}
%
\subsubsection[Fortran 90]{Fortran 90 facilities and subroutines}
\begin{itemize}
\item Improvement of \htmlref{\texttt{query\_disc}}{sub:query_disc} routine in \mylinkext{sub:query_disc:inclusive}{\tt inclusive} mode,
\item the routines 
...
\subsubsection[IDL]{IDL}
\begin{itemize}
\item Improvement of \htmlref{\texttt{query\_disc}}{idl:query_disc} routine in \mylinkext{idl:query_disc:inclusive}{\tt inclusive} mode;
\item update of the required
    \htmladdnormallink{\texttt{IDL-astron} library}{https://idlastro.gsfc.nasa.gov/homepage.html}
    routines, and \htmladdnormallink{\texttt{Coyote}}{http://www.idlcoyote.com} library
    routines (2021-04-08).%
...
\begin{itemize}
\item Switch to 
	\htmladdnormallink{\texttt{healpy 1.15.0}}{\hpysite/releases} 
	(\htmladdnormallink{CHANGELOG}{\hpysite/blob/master/CHANGELOG.rst})
\begin{itemize}
\item \texttt{write\_map} keeps dtype of input map array instead of float32; \texttt{read\_map} keeps dtype of FITS file instead of upcasting to float64; \texttt{write\_cl} uses dtype of input cl instead of float64
\item Changed all warnings to using the \texttt{logging} module, deprecated all \texttt{verbose} keywords
\item Flip sign for spin-0 \texttt{alm2map\_spin} and \texttt{map2alm\_spin}; fixed \texttt{map2alm\_spin} bug for masked input
\item Support transparency in plotting with the \texttt{alpha} parameter
\item Experimental \texttt{projview} function to plot maps using projections from \texttt{matplotlib}
\item Removed the note that we will change order of cl in \texttt{synfast} and \texttt{synalm}, we will leave \texttt{new=False} default
\item Added convenience functions \texttt{order2npix} and \texttt{npix2order}
\item Support nested maps \texttt{hp.smoothing}; fixed indexing issue in \texttt{bl2beam}
\item Allow OBJECT FITS header not to be a string
\item Drop support for Python 2.7-3.5; Improvements of the build system; Automatically build wheels for Linux/MacOS on Github actions
\item and other minor bug fixes ...
\end{itemize}
\end{itemize}
%
% ------
%----------------------------------------------------
\section{Appendix II: Older changes (versions 3.00 to 3.70)}
%
{\mytiny{%
%
\tinysubsection*{Bug corrections and Improvements in Version 3.70 (2020-07)}%2020-07-24
\label{sec:v361}\mytarget{install:v361}
\label{sec:v370}\mytarget{install:v370}
%
\tinysubsubsection*{General}
\begin{itemize}
   \item Fixed several bugs in the \mylink{install:configure}{\texttt{configure}} script
   \item Documentation now available in \mylink{install:healpix_doc}{EPUB format}
\end{itemize}
\tinysubsubsection*{Fortran 90 \linklatexhtml{facilities}{facilities.pdf}{facilities.htm} and
 	\linklatexhtml{subroutines}{subroutines.pdf}{subroutines.htm}}
\begin{itemize}
    \item Addition of the subroutines
    \htmlref{\texttt{read\_fits\_partial}}{sub:read_fits_partial} and 
    \htmlref{\texttt{write\_fits\_partial}}{sub:write_fits_partial} 
    to read and write FITS files containing polarized or unpolarized maps defined on a fraction of the sky (see \url{\hpxfitsdoc}).
\end{itemize}
%
...
\begin{itemize}
\item Switch to 
	\htmladdnormallink{\texttt{healpy 1.14.0}}{\hpysite/releases} 
	(\htmladdnormallink{CHANGELOG}{\hpysite/blob/master/CHANGELOG.rst})
\begin{itemize}
\item Line Integral Convolution plots to plot polarization,
\item fixed FITS files that were left open,
\item increased precision in coordinate transforms,
\item fix propagation on mmax in smoothing,
\item reworked verbose,
\item and many other improvements and bugs fixes ...
\end{itemize}
\end{itemize}
% END of optional healpy
%---------------
\tinysubsection*{Bug corrections and Improvements in Version 3.60 (2019-12)}%2019-12
\label{sec:v360}
...
\begin{itemize}
\item Switch to 
	\htmladdnormallink{\texttt{healpy 1.13.0}}{\hpysite/releases} 
	(\htmladdnormallink{CHANGELOG}{\hpysite/blob/master/CHANGELOG.rst})
\begin{itemize}
...
\tinysubsection*{Bug corrections and Improvements in Version 3.50 (2018-11)}%2018-11
...
\begin{itemize}
\item Switch to 
	\htmladdnormallink{\texttt{healpy 1.12.8}}{\hpysite/releases} 
	(\htmladdnormallink{CHANGELOG}{\hpysite/blob/master/CHANGELOG.rst})
\end{itemize}
% ------
...
\begin{itemize}
\item Switch to 
	\htmladdnormallink{\texttt{healpy 1.12.0}}{\hpysite/releases} 
	(\htmladdnormallink{CHANGELOG}{\hpysite/blob/master/CHANGELOG.rst})
...
directly in future projects making use of Java \healpixns.
...
% 	\setlength{\itemsep}{0pt}
% 	\setlength{\parsep}{0pt}
% 	\item 64 bit java-\healpixns, supports $\nside$ up to $ = 2^{29}$ = 536870912
% 	\begin{itemize}
% 	\setlength{\itemsep}{0pt}
% 	\setlength{\parsep}{0pt}
