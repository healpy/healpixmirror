...
for the package (see \linklatexhtml{the Installation Document}{install.pdf}{install.html}).


\subsection{Using \healpix IDL together with other IDL libraries}
\label{idl:other_idl_libs}
\mylink{idl:other_idl_libs}
...
See the section ``\htmlref{Using GDL instead of IDL}{install:using_gdl}'' in \linklatexhtml{the Installation Document}{install.pdf}{install.html} for more information on this issue.
...
origin and type of character font;
 \item the \mylink{idl:mollview:crop}{{\tt CROP}} keyword now has the same behavior for all output media (GIF, JPEG, PDF, PNG, PS, ... and X);  % 
  the \mylink{idl:mollview:nobar}{{\tt NOBAR}} keyword now removes the color bar {\em or} the polarization color wheel, as applicable; correct EQUINOX date in header of output \mylink{idl:mollview:fits}{FITS} map.
\end{itemize}
%
