...
nsmax\mytarget{sub:mpi_initialize_alm_tools:nsmax} & I4B & IN & the $N_{side}$ value of the HEALPix map. (OPTIONAL) \\
nlmax\mytarget{sub:mpi_initialize_alm_tools:nlmax} & I4B & IN & the maximum $\ell$ value used for the $a_{lm}$. (OPTIONAL) \\
nmmax\mytarget{sub:mpi_initialize_alm_tools:nmmax} & I4B & IN & the maximum $m$ value used for the $a_{lm}$. (OPTIONAL) \\
\end{tabular}
\begin{tabular}{p{0.4\hsize} p{0.05\hsize} p{0.05\hsize} p{0.40\hsize}} \hline  
zbounds\mytarget{sub:mpi_initialize_alm_tools:zbounds}(1:2) & DP & IN & section of the map on which to perform the $a_{lm}$
