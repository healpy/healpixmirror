...
(\htmladdnormallink{{\tt https://lambda.gsfc.nasa.gov/toolbox/tb\_cmbfast\_ov.cfm}}
...
{\tt camb} and {\tt class}:
\begin{itemize}
\item {\tt camb} (\htmladdnormallink{{\tt http://camb.info}}{http://camb.info}) 
...
\item {\tt class} (\htmladdnormallink{{\tt http://class-code.net}}{http://class-code.net}) 
is written in C and C++, and only outputs $\frac{\ell(\ell+1)}{2\pi}C_X(\ell)$ in plain text files 
(optionally in [$\mu$K]$^2$ and in a order of columns for polarized spectra matching the one of {\tt camb}).
\end{itemize}
