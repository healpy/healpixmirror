...
\usepackage{xr-hyper,healpix,html,graphicx,makeidx,color,natbib,amsmath} %heqn
%\usepackage{natbib,healpix,html,graphicx,makeidx,color,xr-hyper} %heqn
...
% \textbf{Boomerang}
% (see \url{http://cmb.PHYS.cwru.edu/boomerang/} or 
% \url{http://oberon.roma1.infn.it/boomerang/})
% and \textbf{Archeops} (
% \url{http://archeops01.free.fr/main_archeops/index_english.html}).
...
\htmladdnormallink{version 2 of the GNU General Public License (\textbf{GPLv2})}%
...
\htmladdnormallink{\textbf{3-clause BSD license}}{https://opensource.org/licenses/BSD-3-Clause}.
...
\label{sec:conventions}%\warnhtml
A bandlimited function $f$ on the sphere can
be expanded in spherical harmonics, $Y_{\ell m}$,
as
\begin{align}
  \label{eq:alms}
  & f({ \gamma}) = \sum_{\ell =0}^{\lmax}\sum_{m}a_{\ell m}Y_{\ell m}(\gamma),%%%  f({ \gamma})&\myequal&\sum_{\ell =0}^{\lmax}\sum_{m}a_{\ell m}Y_{\ell m}(\gamma),
...
such as \htmlref{\texttt{anafast}}{fac:anafast} or \texttt{anafast\_cxx}.
...
\htmlref{\texttt{synfast}}{fac:synfast}, 
...
% we provide a new version of the file \texttt{fitsout.f} used by CMBFAST, so that
...
(\htmlref{\texttt{convert\_oldhpx2cmbfast}}{idl:convert_oldhpx2cmbfast}) 
...
In fact $X_{1,lm}(\vecn)$ and $X_{2,lm}(\vecn)$ have the form,
$\myhtmlimage{} {X_{1,lm}(\vecn)=\sqrt{(2l+1) / 4\pi} F_{1,lm}(\theta)\ e^{im\phi}}$
and $\myhtmlimage{} {X_{2,lm}(\vecn)=\sqrt{(2l+1) / 4\pi} F_{2,lm}(\theta)\ e^{im\phi}}$, 
...
where 
\begin{eqnarray}
   N_{lm}(\theta) &=& 2 \sqrt{(l-2)!(l-m)! \over (l+2)!(l+m)!}.\myhtmlimage{}
\end{eqnarray}
...
\begin{eqnarray}
\sum_m \;_{s_1} Y_{lm}^*(\vecn_1) 
\;_{s_2} Y_{lm}(\vecn_2) &=& \sqrt{2l+1 \over 4 \pi} 
...
\langle T(1)U_{r}(2) \rangle &=& 0.\myhtmlimage{}
\label{QUr}
\end{eqnarray}
The subscript $r$ 
...
Table 1 summarizes the relations between the CMB power spectra in the different
releases. 
...
% \vskip 0.3cm
% \noindent\textbf{How \healpix deals with these discrepancies}\\
\subsubsection{How \healpix deals with these discrepancies: \texttt{POLCCONV} keyword}
\label{intro:polcconv}
The FITS keyword \texttt{POLCCONV} has been introduced in \healpix 2.0 to describe the
...
(default for sky map synthetized with HEALPix routine \htmlref{\texttt{synfast}}{fac:synfast})
...
% \htmlref{\texttt{smoothing}}{fac:smoothing} will stop with an error message if the map to analyze follows the 'IAU' convention, or if the convention is explicitely set to a value different from 'COSMO' and 'IAU'.\\
...
-- the F90 subroutine \htmlref{\texttt{input\_map}}{sub:input_map} in its default mode, \\
-- the F90 facilities calling it and dealing with the $I$, $Q$ and $U$ Stokes parameters as a whole, {\em ie}
\htmlref{\texttt{anafast}}{fac:anafast} and
\htmlref{\texttt{smoothing}}{fac:smoothing}, \\
-- as well as their IDL wrappers 
\htmlref{\texttt{ianafast}}{idl:ianafast} and
...
called with 
\mylinkext{idl:mollview:polarization}{\texttt{Polarization=2} or \texttt{3}},\\
-- and all C++ facilities (and the input routine \texttt{read\_Healpix\_map\_from\_fits})\\
will all \\
-- issue an error message and 
crash if \texttt{POLCCONV} is explicitely set to a value different from \texttt{'COSMO'} and \texttt{'IAU'},\\
-- issue a warning (except in C++), and {\em swap the sign of the $U$ polarisation} stored into memory if the FITS file being read contains \texttt{POLCCONV='IAU'}, \\
-- issue a warning (except in C++) if the keyword \texttt{POLCCONV} is totally absent, and then carry on with the original data,\\
-- or work silently with the original data if \texttt{POLCCONV='COSMO'}.\\
On the other hand, and as in previous releases, routines treating or showing 
each of $I$, $Q$ and $U$ fields separately, 
such as the F90 facilities
\htmlref{\texttt{median\_filter}}{fac:median_filter},
\htmlref{\texttt{ud\_grade}}{fac:ud_grade}, or
\htmlref{\texttt{map2gif}}{fac:map2gif}
as well as their IDL counterparts
\htmlref{\texttt{median\_filter}}{idl:median_filter},
\htmlref{\texttt{ud\_grade}}{idl:ud_grade}, or
\htmlref{\texttt{mollview} et al}{idl:mollview} run with 
\mylinkext{idl:mollview:polarization}{\texttt{Polarization=0} or \texttt{1}} will
ignore the value of \texttt{POLCCONV} (copying it unchanged into their output files, when applicable) 
and preserve the sign of $U$.

Finally, 
the IDL subroutine 
\htmlref{\texttt{change\_polcconv.pro}}{idl:change_polcconv} 
and the Python facility 
\htmladdnormallink{\texttt{change\_polcconv.py}}{\srcurl src/python/change_polcconv.py} are 
...
in Fortran90 and C++. 
It resides in 
\htmladdnormallink{\texttt{src/f90/mod/rngmod.f90}}{\srcurl src/f90/mod/rngmod.f90}
and 
\htmladdnormallink{\texttt{src/cxx/cxxsupport/planck\_rng.h}}{\srcurl src/cxx/cxxsupport/planck_rng.h}
and supersedes the previous RNG (which is still available at 
\htmladdnormallink{\texttt{src/f90/mod/ran\_tools\_dist.f90}}{\srcurl src/f90/mod/ran_tools_dist.f90}). 
...
\latexhtml{\phantomsection
\addcontentsline{toc}{section}{References}}{}
\begin{thebibliography}{99}
%
\backrefparscanfalse%because automatic pagebackref does not work properly
...
\backrefprint%because automatic pagebackref does not work properly
%
...
\htmladdnormallink{MNRAS, 350, 914}{http://adsabs.harvard.edu/abs/2004MNRAS.350..914C}.
...
Crittenden, R. and Turok, N.G., 1998, \htmladdnormallink{astro-ph/9806374}{https://arxiv.org/abs/astro-ph/9806374}
\backrefprint%because automatic pagebackref does not work properly
...
Christensen, P.R. \&
Chiang,	     L.-Y., 2005,
IJMPD, 14, 2, 275, \htmladdnormallink{astro-ph/0305537}{https://arxiv.org/abs/astro-ph/0305537}
\backrefprint%because automatic pagebackref does not work properly
...
\bibitem[G{\oacute}rski {et~al.}(2005)]{gorskihealpix05}
...
\htmladdnormallink{astro-ph/0409513}{https://arxiv.org/abs/astro-ph/0409513}
\backrefprint%because automatic pagebackref does not work properly
...
2002, \htmladdnormallink{ApJ, 567, 2}{http://adsabs.harvard.edu/abs/2002ApJ...567....2H}, 
\htmladdnormallink{astro-ph/0105302}{https://arxiv.org/abs/astro-ph/0105302}
\backrefprint%because automatic pagebackref does not work properly
...
Planck Collaboration, 2015-XI, \htmladdnormallink{arXiv:1507.02704v1}{https://arxiv.org/abs/1507.02704v1}
\backrefprint%because automatic pagebackref does not work properly

\bibitem[Reinecke \& Hivon(2015)]{rh15}
Reinecke, M. and Hivon, E., 2015, 
\htmladdnormallink{A\&A, 580, A132}{http://adsabs.harvard.edu/abs/2015A\%26A...580A.132R}, 
\htmladdnormallink{arXiv:1505.04632v2}{https://arxiv.org/abs/1505.04632}
\backrefprint%because automatic pagebackref does not work properly

\bibitem[Rocha {et~al.}(2009)]{xfaster}
Rocha, G., Contaldi, C.R., Bond, J.R. \& G{\oacute}rski, K.M., 2009,
\htmladdnormallink{arXiv:0912.4059v2}{https://arxiv.org/abs/0912.4059}
\backrefprint%because automatic pagebackref does not work properly
...
\htmladdnormallink{astro-ph/9812335}{https://arxiv.org/abs/astro-ph/9812335}, to appear in 
...
\htmladdnormallink{MNRAS, 358, 833}{http://adsabs.harvard.edu/abs/2005MNRAS.358..833T}
\backrefprint%because automatic pagebackref does not work properly

\bibitem[Wandelt, Hivon \& G{\oacute}rski(1998)]{whg}
Wandelt, B.D., Hivon, E. and G{\oacute}rski, K.M., 1998, 
\htmladdnormallink{astro-ph/9803317}{https://arxiv.org/abs/astro-ph/9803317}, in
 "Fundamental Parameters in Cosmology", proceedings of the XXXIIIrd Rencontres
de Moriond, Tr{\^a}n Thanh V{\^a}n (ed.)
\backrefprint%because automatic pagebackref does not work properly

\bibitem[Wandelt, Hivon \& G{\oacute}rski(2001)]{whg2001}
Wandelt, B.D., Hivon, E. and G{\oacute}rski, K.M., 2001, 
\htmladdnormallink{Phys. Rev. D, 64, 083003}{http://adsabs.harvard.edu/abs/2001PhRvD..64h3003W},
\htmladdnormallink{astro-ph/9808292v1}{https://arxiv.org/abs/astro-ph/9808292v1}
\backrefprint%because automatic pagebackref does not work properly

%% \bibitem{fastah} Wandelt, B.D., G{\oacute}rski, K.M., Hivon, E.\ and Hansen,
%% F.\, in preparation
