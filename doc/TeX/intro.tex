...
(or quad-sphere, see 
\htmladdnormallink{\texttt{http://space.gsfc.nasa.gov/ast\-ro/co\-be/skymap\_info.html}}
...
FITS I/O interfaces 
(\htmladdnormallink{\texttt{http://heasarc.gsfc.nasa.gov/docs/software/fitsio}}
...
(see \htmladdnormallink{\texttt{http://www.cosmos.esa.int/web/planck}}%
...
\begin{itemize}
\item \texttt{camb} (\htmladdnormallink{\texttt{http://camb.info}}{http://camb.info}) 
...
interpreted as meaning \texttt{'COSMO'}.
...
Starting with \healpix 3.4, when dealing with a polarized (full-sky or cut-sky) signal map, \\
...
-- and the IDL visualisation routines 
...
-- issue a warning, and {\em swap the sign of the $U$ polarisation} stored into memory if the FITS file being read contains \texttt{POLCCONV='IAU'}, \\
-- issue a warning if the keyword \texttt{POLCCONV} is totally absent, and then carry on with the original data,\\
-- work silently with the original data if \texttt{POLCCONV='COSMO'}.\\
On the other hand, routines treating each of $I$, $Q$ and $U$ fields separately, 
such as the F90 facilities
\htmlref{\texttt{median\_filter}}{fac:median_filter} and
\htmlref{\texttt{ud\_grade}}{fac:ud_grade}, as well as their IDL implementation 
\htmlref{\texttt{median\_filter}}{idl:median_filter} and
\htmlref{\texttt{ud\_grade}}{idl:ud_grade}
ignore the value of \texttt{POLCCONV} and copy it unchanged into their output files, as before.

Finally, the IDL facility \htmlref{\texttt{change\_polcconv}}{idl:change_polcconv} is provided to add the 
\texttt{POLCCONV} keyword or
...
in Fortran90 and C++. 
It resides in 
\htmladdnormallink{\texttt{src/f90/mod/rngmod.f90}}{http://sourceforge.net/p/healpix/code/HEAD/tree/trunk/src/f90/mod/rngmod.f90}
and 
\htmladdnormallink{\texttt{src/cxx/cxxsupport/planck\_rng.h}}{http://sourceforge.net/p/healpix/code/HEAD/tree/trunk/src/cxx/cxxsupport/planck_rng.h}
and supersedes the previous RNG (which is still available at 
\htmladdnormallink{\texttt{src/f90/mod/ran\_tools\_dist.f90}}{http://sourceforge.net/p/healpix/code/HEAD/tree/trunk/src/f90/mod/ran_tools_dist.f90}). 
...
(cf. \htmladdnormallink{\texttt{http://www.cs.hku.hk/}}{http://www.cs.hku.hk/}).
