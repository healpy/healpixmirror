...
Since some applications require to mix pixels of different resolutions, it is possible
to merge the resolution $\nside$ and index $p$ into a single unique number $u$
\begin{eqnarray}
	u\myhtmlimage{} &\myequal& p + 4 \nside^2, \myhtmlimage{} \label{eq:nest2uniq}
\end{eqnarray}
so that all pixels with $\nside=1$ have the unique indices 4 to 15, those with $\nside=2$ are in the range 16 to 63, and so on \cite{rh15}, simplifying the handling of data with heteregenous $\nside$.
Casting the unique index $u$ into its original $\nside$ and $p$ is simply
\begin{eqnarray}
	\nside &\myequal& 2^{\rm{floor}\left( \log_2(u/4)/2 \right)}, \myhtmlimage{} \label{eq:ring2nest_a}\\
	     p\myhtmlimage{} &\myequal& u - 4 \nside^2, \myhtmlimage{} \label{eq:ring2nest_b}
\end{eqnarray}
giving immediate access to the usual pixel based tools.\\
This unique indexing can in principle be applied to both the RING and NESTED schemes, 
even though the latter is likely to be more relevant for a hierarchical description of data with variable resolutions. It is for instance the basis of the NUNIQ (Nested Unique) storage scheme used for Multi-Order Coverage map (MOC) description of astronomical datasets proposed for virtual observatories \cite{moc}.\\
...
Christensen, P.R. \&
Chiang,	     L.-Y., 2005,
IJMPD, 14, 2, 275, astro-ph/0305537
...
Reinecke, M. and Hivon, E., 2015, A\&A in press, 
